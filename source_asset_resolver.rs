...
impl MountedModelResolver {
    ...
    pub fn resolve(&self, source_path: &str) -> Result<ResolvedStudioModel, String> {
        ...
        let package_content_hash =
            hash_model_package(&mdl, &vvd, &vtx, ani.as_deref(), phy.as_deref());
        ...
        match &asset.source {
            ...
            IndexedSource::Vpk {
                ...
                let start = usize::try_from(*offset)
                    .map_err(|_| "VPK offset does not fit usize".to_string())?;
                let end = start
                    .checked_add(*length)
...
fn hash_model_package(
    ...
    for bytes in [Some(mdl), Some(vvd), Some(vtx), ani, phy]
        .into_iter()
        .flatten()
    {
...
mod tests {
    ...
    fn model_hash_is_deterministic() {
        let a = hash_model_package(b"mdl", b"vvd", b"vtx", None, None);
        let b = hash_model_package(b"mdl", b"vvd", b"vtx", None, None);
