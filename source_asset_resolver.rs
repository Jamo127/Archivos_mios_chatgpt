...
use crate::studiomodel::{STUDIO_MODEL_PACKAGE_VERSION, StudioModelInput};
...
impl ResolvedStudioModel {
    pub fn as_input(&self, skin: usize) -> StudioModelInput<'_> {
        StudioModelInput {
            source_path: &self.source_path,
            mdl: &self.mdl,
            vvd: &self.vvd,
            vtx: &self.vtx,
            ani: self.ani.as_deref(),
            phy: self.phy.as_deref(),
            skin,
        }
    }
}
...
fn collect_directory_assets(
    ...
    for entry in entries {
        ...
        let logical_path = format!("models/{}", relative.to_string_lossy());
        let normalized = normalize_model_resource_path(&logical_path)?;
